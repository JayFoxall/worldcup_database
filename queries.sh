...
echo "$($PSQL "SELECT name FROM games FULL JOIN teams ON games.winner_id = teams.team_id OR games.opponent_id = teams.team_id WHERE games.round ILIKE 'Eighth%' AND games.year = 2014 ORDER BY name")" 

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT(name) AS unique_winners FROM games INNER JOIN teams on games.winner_id = teams.team_id ORDER BY unique_winners")" 
